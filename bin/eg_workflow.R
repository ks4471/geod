...
dummy=geo.query('/Users/ks907/Downloads/geo_datasets_series.txt')		## location of the file from GEO

#write.table(dummy,file='')


	head(matst(dummy$type))
dummy=dummy[dummy$type=='Expression profiling by array',]


#query=c('panobinostat','deferoxamine')
query=c('panobinostat','deferoxamine')

ans=grepl(paste(query,collapse="|"),dummy$title) |  grepl(paste(query,collapse="|"),dummy$full)

holder=dummy[ans,]

#write.table(holder,file='')


## retrieve datasets by GEO id, currently works only for linux
geo.matrix('GSE26790',path='/Users/ks907/Downloads/')
