...
cseq=pltin[grepl('quantseq|lexogen',pltin$title)&pltin$species%in%c('homo sapiens','mus musculus','rattus norvegicus'),]

quer1=c('quantseq','lexogen')
#query=c(' single.nucleus',' individual.nucleus',' nuclear.seq')
# quer2=c('brain ',' neuron','nervous','neural',"gabaergic","glutamatergic","cerebral","cerebellum","hippocampus","somatosensory")
# qnot1=c('fetal','esc','ips')
...
stuffs=pltin[
		 (
		   (grepl(paste(quer1,collapse='|'),pltin$title)	| grepl(paste(quer1,collapse='|'),pltin$main))
		 # & (grepl(paste(quer2,collapse='|'),pltin$title)	| grepl(paste(quer2,collapse='|'),pltin$main))
		 # &!(grepl(paste(qnot1,collapse='|'),pltin$title)	| grepl(paste(qnot1,collapse='|'),pltin$main))
		   & (grepl(paste(qspec,collapse='|'),pltin$species))
		 ),]
	str(stuffs)
